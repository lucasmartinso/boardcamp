...
export async function validateGames(req,res,next) { 
    ...
    const findExist = await connection.query('SELECT * FROM categories WHERE id= $1',[categoryId]);
    ...
    try {
        const findRepetead = await connection.query('SELECT * FROM games WHERE name= $1',[name]); 
        if(findRepetead) { 
